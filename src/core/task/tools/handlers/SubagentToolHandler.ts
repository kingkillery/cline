...
export class UseSubagentsToolHandler implements IFullyManagedTool {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		const updateEntryFromProgress = async (
			index: number,
			update: Awaited<ReturnType<SubagentRunner["run"]>> | Parameters<Parameters<SubagentRunner["run"]>[1]>[0],
		) => {
		...
		const execution = prompts.map((prompt, index) =>
			runners[index].run(prompt, async (update) => updateEntryFromProgress(index, update)),
		)
