...
function parseDraftNodeNumber(id: string): number | null {
	const match = /^draft-(\d+)$/.exec(id.trim());
	if (!match) {
		return null;
	}
...
export function createNextDraftNodeId(graph: NativeToolDraftGraph | null): string {
	const maxNodeNumber = (graph?.nodes ?? []).reduce((max, node) => {
		const parsed = parseDraftNodeNumber(node.id);
		return parsed && parsed > max ? parsed : max;
	}, 0);
	return `draft-${maxNodeNumber + 1}`;
}
...
export function validateNativeToolDraftGraph(graph: NativeToolDraftGraph): string[] {
	...
	for (const node of graph.nodes) {
		const duplicateCount = graph.nodes.filter((candidate) => candidate.id === node.id).length;
		if (duplicateCount > 1) {
			errors.push(`Duplicate draft task id "${node.id}".`);
			continue;
		}
